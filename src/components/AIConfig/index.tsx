...
const QUICKSTART_PROVIDER_IDS = ['openai', 'anthropic', 'deepseek', 'openrouter', 'ollama'];

const PROVIDER_GUIDES: Record<string, ProviderGuide> = {
  openai: {
    title: '通用首选',
    summary: '如果你只想先跑起来，一般从 OpenAI 开始最稳。',
    setupHint: '准备 API Key，地址通常保持默认。',
    fitFor: '适合第一次接触 OpenClaw 的用户',
  },
  anthropic: {
    title: '复杂任务强',
    summary: '更适合长上下文、复杂指令和代码理解。',
    setupHint: '准备 Anthropic Key，优先选推荐模型。',
    fitFor: '适合重视回复质量的用户',
  },
  deepseek: {
    title: '中文友好',
    summary: '成本和效果通常更平衡，中文体验也不错。',
    setupHint: '准备 DeepSeek Key，先用 deepseek-chat 即可。',
    fitFor: '适合中文日常使用',
  },
  openrouter: {
    title: '模型多',
    summary: '一个入口管理多家模型，方便以后切换。',
    setupHint: '准备 OpenRouter Key，模型名称通常更长。',
    fitFor: '适合想经常换模型的人',
  },
  ollama: {
    title: '本地模型',
    summary: '不走外部 API，适合本地部署场景。',
    setupHint: '先确认 Ollama 本机已经可用。',
    fitFor: '适合在本机跑模型的用户',
  },
};
...
const formatContextWindow = (value: number | null) => {
  ...
  if (value >= 1000000) return `上下文 ${(value / 1000000).toFixed(1)}M`;
  if (value >= 1000) return `上下文 ${Math.round(value / 1000)}K`;
  return `上下文 ${value}`;
};

const findOfficialProvider = (
  officialProviders: OfficialProvider[],
  providerId: string | null | undefined
) => {
  if (!providerId) return null;
  return officialProviders.find((provider) => provider.id === providerId) || null;
};
...
function ProviderDialog({
  ...
  const initialOfficial = isEditing
    ? officialProviders.find(p => editingProvider && (editingProvider.name.includes(p.id) || p.id === editingProvider.name)) || null
    : findOfficialProvider(officialProviders, preferredProviderId);
  const [step, setStep] = useState<'select' | 'configure'>(
    isEditing || !!initialOfficial ? 'configure' : 'select'
  );
  ...
  // 配置表单
  const [providerName, setProviderName] = useState(editingProvider?.name || initialOfficial?.id || '');
  const [baseUrl, setBaseUrl] = useState(editingProvider?.base_url || initialOfficial?.default_base_url || '');
  ...
  const [apiType, setApiType] = useState(() => {
    ...
    return initialOfficial?.api_type || 'openai-completions';
  });
  ...
  const providerGuide = selectedOfficial ? PROVIDER_GUIDES[selectedOfficial.id] : null;
  const quickStartProviders = officialProviders.filter(provider =>
    QUICKSTART_PROVIDER_IDS.includes(provider.id)
  );
      ...
      >
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(85vh-140px)]">
              ...
              >
                ...
                {quickStartProviders.length > 0 && (
                  <div className="space-y-3 pt-4 border-t border-dark-600">
                    <h3 className="text-sm font-medium text-gray-400">新手常用</h3>
                    <div className="grid gap-3">
                      {quickStartProviders.map(provider => {
                        const guide = PROVIDER_GUIDES[provider.id];
                        return (
                          <button
                            key={`quick-${provider.id}`}
                            onClick={() => handleSelectOfficial(provider)}
                            className="flex items-start gap-3 p-4 rounded-xl bg-dark-700/70 border border-dark-500 hover:border-claw-500/50 hover:bg-dark-600 transition-all text-left"
                          >
                            <span className="text-2xl">{provider.icon}</span>
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2">
                                ...
                                {guide && (
                                  <span className="text-[11px] px-2 py-0.5 rounded-full bg-claw-500/15 text-claw-300">
                                    {guide.title}
                                  </span>
                                )}
                              </div>
                              <p className="text-xs text-gray-400 mt-1">
                                {guide?.summary || '选择后按引导填入 Key 即可开始使用'}
                              </p>
                            </div>
                            <ChevronRight size={16} className="text-gray-500 group-hover:text-claw-400 transition-colors" />
                          </button>
          ...
          <motion.div
              ...
              >
                {selectedOfficial && (
                  <div className="rounded-xl border border-claw-500/20 bg-claw-500/8 p-4 space-y-3">
                    <div className="flex items-center gap-3">
                      <span className="text-2xl">{selectedOfficial.icon}</span>
                      <div>
                        <p className="text-white font-medium">{selectedOfficial.name}</p>
                        <p className="text-sm text-gray-400">
                          {providerGuide?.summary || '按推荐步骤填写后即可保存'}
                        </p>
                      </div>
                    </div>
                    <div className="grid gap-2 text-sm text-gray-300 md:grid-cols-3">
                      <div className="rounded-lg bg-dark-700/70 p-3">
                        <p className="text-xs text-gray-500 mb-1">步骤 1</p>
                        <p>填入 API Key</p>
                      </div>
                      <div className="rounded-lg bg-dark-700/70 p-3">
                        <p className="text-xs text-gray-500 mb-1">步骤 2</p>
                        <p>保持默认地址或按需修改</p>
                      </div>
                      <div className="rounded-lg bg-dark-700/70 p-3">
                        <p className="text-xs text-gray-500 mb-1">步骤 3</p>
                        <p>选择推荐模型后保存</p>
                      </div>
                    </div>
                    {providerGuide && (
                      <p className="text-xs text-claw-300">{providerGuide.setupHint}</p>
                    )}
                  </div>
                )}
                ...
                <div>
                  <label className="block text-sm text-gray-400 mb-2">
                    服务标识
                    <span className="text-gray-600 text-xs ml-2">(内部使用，如无特殊需求保持默认)</span>
                  </label>
                  <input
                    type="text"
                    value={providerName}
                ...
                <div>
                  ...
                  />
                  {selectedOfficial?.default_base_url && (
                    <p className="text-xs text-gray-500 mt-1">
                      一般保持默认即可。只有接第三方兼容接口时才需要修改。
                    </p>
                  )}
                </div>
...
export function AIConfig() {
  ...
  const openProviderWizard = (providerId?: string | null) => {
    setPreferredProviderId(providerId ?? null);
  ...
  return (
    <div className="h-full overflow-y-auto scroll-container pr-2">
      <div className="max-w-4xl space-y-6">
        ...
        {quickStartProviders.length > 0 && (
          <div className="bg-dark-700/70 rounded-2xl border border-dark-500 p-6 space-y-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 className="text-lg font-medium text-white">新手快速开始</h3>
                <p className="text-sm text-gray-500 mt-1">
                  先选一个常用服务，再按向导填 Key 和模型，通常 1 分钟内可以完成。
                </p>
              </div>
              <button
                onClick={() => openProviderWizard()}
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {quickStartProviders.map(provider => {
                ...
                return (
                  <button
                    key={`overview-${provider.id}`}
                    onClick={() => openProviderWizard(provider.id)}
                    className="rounded-xl border border-dark-500 bg-dark-600/70 p-4 text-left hover:border-claw-500/40 hover:bg-dark-600 transition-all group"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          {guide && (
                            <p className="text-xs text-claw-300 mt-0.5">{guide.title}</p>
                          )}
                        </div>
                      </div>
                      <ArrowRight
                        size={16}
                        className="text-gray-500 group-hover:text-claw-300 transition-colors"
                      />
                    </div>
                    <p className="text-sm text-gray-400 mt-3">
                      {guide?.summary || '按引导完成配置后即可加入 OpenClaw。'}
                    </p>
                    {guide && (
                      <p className="text-xs text-gray-500 mt-2">{guide.fitFor}</p>
                    )}
                    <div className="mt-4 flex items-center justify-between text-xs text-gray-500">
                      <span>{provider.suggested_models.length} 个推荐模型</span>
                      <span className="text-claw-300">开始配置</span>
                    </div>
                  </button>
                );
              })}
        ...
        <div className="space-y-4">
          ...
          {aiConfig?.configured_providers.length === 0 ? (
            <div className="bg-dark-700 rounded-xl border border-dark-500 p-6 space-y-6">
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-2xl bg-dark-600 flex items-center justify-center shrink-0">
                  <Sparkles size={24} className="text-claw-400" />
                </div>
                <div>
                  <p className="text-white font-medium">还没有配置 AI Provider</p>
                  <p className="text-sm text-gray-400 mt-1">
                    先添加一个 AI 服务，OpenClaw 才能开始推理和回复消息。
                  </p>
                </div>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-3">
                <div className="rounded-xl border border-dark-500 bg-dark-600/60 p-4">
                  <p className="text-xs text-gray-500 mb-2">步骤 1</p>
                  <p className="text-sm text-white">选一个常用 Provider</p>
                  <p className="text-xs text-gray-500 mt-2">OpenAI、Anthropic、DeepSeek 都适合先上手。</p>
                </div>
                <div className="rounded-xl border border-dark-500 bg-dark-600/60 p-4">
                  <p className="text-xs text-gray-500 mb-2">步骤 2</p>
                  <p className="text-sm text-white">填入 API Key</p>
                  <p className="text-xs text-gray-500 mt-2">默认 API 地址通常不用改，避免一开始就改复杂配置。</p>
                </div>
                <div className="rounded-xl border border-dark-500 bg-dark-600/60 p-4">
                  <p className="text-xs text-gray-500 mb-2">步骤 3</p>
                  <p className="text-sm text-white">选推荐模型并测试</p>
                  <p className="text-xs text-gray-500 mt-2">保存后用“测试连接”确认当前主模型已经可用。</p>
                </div>
              </div>

              <div className="flex flex-wrap gap-3">
                {quickStartProviders.slice(0, 3).map(provider => (
                  <button
                    key={`empty-${provider.id}`}
                    onClick={() => openProviderWizard(provider.id)}
                    className="inline-flex items-center gap-2 rounded-xl border border-dark-500 bg-dark-600/70 px-4 py-3 text-sm text-white hover:border-claw-500/40 hover:bg-dark-600 transition-all"
                  >
                    <span className="text-lg">{provider.icon}</span>
                    <span>配置 {provider.name}</span>
